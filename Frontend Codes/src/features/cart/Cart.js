...
import { deleteCartItemAsync, getCartItemsAsync, getCartItemsByUserIdAsync, selectCartItems, updateCartItemAsync } from './cartSlice';
...
export default function Cart() {
  ...
  //  const user = useSelector(selectLoggedInuser);
     ...
     const totalAmount = products.reduce(
      (amount, products) => discountedPrice(products.product) * products.quantity + amount,
      0
    );
     ...
     const handleQuantity=(e,product)=>{
      dispatch(updateCartItemAsync({ id:product.id , quantity :+e.target.value}));
